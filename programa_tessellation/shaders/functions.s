...
vec3 g2Normal(float u, float v, float t0, float t1, float t2) {
	return vec3( normalize( cross( g2Pu(u, v, t0, t1, t2),  g2Pv(u, v, t0, t1, t2))));
}
...
vec3 f2(float u, float v) {
	return vec3(2 *  g2(u - 0.5, v - 0.5, 0, R, r));
}
...
vec3 f2Pu(float u, float v) {
	return vec3((2 *  g2Pu(u - 0.5, v - 0.5, 0, R, r)));
}

vec3 f2Pv(float u, float v) {
	return vec3((2 *  g2Pv(u - 0.5, v - 0.5, 0, R, r)));
}

vec3 f2Normal(float u, float v) {
	return vec3( normalize( cross( f2Pu(u, v),  f2Pv(u, v))));
}
...
vec3 f2PuPu(float u, float v) {
	return vec3((2 *  g2PuPu(u - 0.5, v - 0.5, 0, R, r)));
}
...
vec3 f2PvPv(float u, float v) {
	return vec3((2 *  g2PvPv(u - 0.5, v - 0.5, 0, R, r)));
}

vec3 f2PuPv(float u, float v) {
	return vec3((2 *  g2PuPv(u - 0.5, v - 0.5, 0, R, r)));
}
...
vec3 fAmbas(float u, float v, float t0) {
	return vec3(t0 *  f(u, v) + (1 - t0) *  f2(u, v));
}

vec3 fAmbasPu(float u, float v, float t0) {
	return vec3(((t0 *  fPu(u, v)) + ((1 - t0) *  f2Pu(u, v))));
}

vec3 fAmbasPv(float u, float v, float t0) {
	return vec3(((t0 *  fPv(u, v)) + ((1 - t0) *  f2Pv(u, v))));
}

vec3 fAmbasNormal(float u, float v, float t0) {
	return vec3( normalize( cross( fAmbasPu(u, v, t0),  fAmbasPv(u, v, t0))));
}
...
vec3 fAmbasPuPu(float u, float v, float t0) {
	return vec3(((t0 *  fPuPu(u, v)) + ((1 - t0) *  f2PuPu(u, v))));
}

vec3 f2PvPu(float u, float v) {
	return vec3((2 *  g2PvPu(u - 0.5, v - 0.5, 0, R, r)));
}
...
vec3 fAmbasPvPv(float u, float v, float t0) {
	return vec3(((t0 *  fPvPv(u, v)) + ((1 - t0) *  f2PvPv(u, v))));
}

vec3 fAmbasPuPv(float u, float v, float t0) {
	return vec3(((t0 *  fPuPv(u, v)) + ((1 - t0) *  f2PuPv(u, v))));
}
...
vec3 tangentParam(vec2 p) {
	if (funPlot==0) {
		return fAmbasPu(p.x, p.y, param_t[0]);
	}
...
vec3 cotangentParam(vec2 p) {
	if (funPlot==0) {
		return fAmbasPv(p.x, p.y, param_t[0]);
	}
...
float areaParam(vec2 p) {
	if (funPlot==0) {
		return fAmbasArea(p.x, p.y, param_t[0]);
	}
...
float curvatureParam(vec2 p) {
	if (funPlot==0) {
		return fAmbasK(p.x, p.y, param_t[0]);
	}
