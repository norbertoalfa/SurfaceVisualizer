...
// ---------------------------------------------------------------------
// Iniciaización de OpenGL (se hace a cada cuadro y al incio)
...
void InicializaOpenGLCuadro()
{
   ...
   glLineWidth( 1.8 );          // establecer ancho de línea
   glPointSize( 7.5  );         // establecer tamaño de los puntos
   CError();
...
void VisualizarInterfaz()
{
	...
	siguiente_obj += ImGui::Button("Siguiente objeto");
	siguiente_esc += ImGui::Button("Siguiente escena");
...
void FGE_PulsarLevantarTecla( GLFWwindow* window, int key, int scancode, int action, int mods )
{
   ...
   if ( glfwGetKey( window, GLFW_KEY_A ) == GLFW_PRESS )
   {
      ...
      // 'true', forzar revisualizar escena.
      // .....
...
void InicializaGLFW( int argc, char * argv[] )
{
   ...
   ConfigurarGLFW(); // especificar algunos parámetros de GLFW (ver 'aux.cpp')
  ...
  //w2 = glfwCreateWindow( wx, wy, "Interfaz", nullptr, nullptr ); // crea ventana
   ...
   /////////////////////////////////////////////////////////////////////////////
	...
	ImGui_ImplGlfw_InitForOpenGL(ventana_glfw, true);
...
void BucleEventosGLFW()
{
	...
	while ( ! terminar_programa  )
	{
		...
		{              //    procesar todos los eventos pendientes, y llamar a la función correspondiente, si está definida
		...
		if (!revisualizar_escena)       // si no hay una animacion en curso
